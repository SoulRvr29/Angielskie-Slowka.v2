    ...
    const fetchWords = async (id) => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_DOMAIN}/prywatne_zestawy/${id}`
        );
        console.log(
          `${process.env.NEXT_PUBLIC_API_DOMAIN}/prywatne_zestawy/${id}`
        );
  ...
  return (
    <div className="flex flex-col gap-4 items-center justify-center">
      ...
      <div className="flex flex-col border max-sm:border-none max-sm:rounded-none w-fit overflow-hidden min-w-lg max-sm:min-w-auto max-sm:w-full rounded-t-md border-primary/50 bg-primary/10">
        ...
        <Link
          href={`${process.env.NEXT_PUBLIC_DOMAIN}/prywatne_zestawy/${id}/fiszki?size=${size}`}
      ...
      {id !== "zapisane" && (
        <>
          <div className="flex gap-4">
            <Link
              href={`${process.env.NEXT_PUBLIC_DOMAIN}/prywatne_zestawy/${id}/edycja`}
